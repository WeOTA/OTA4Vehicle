(function() {
  ...
  angular
    .module('otaRm')
    .controller('PmcontrolListController', PmcontrolListController)
    .controller('PmcontrolDetailController',PmcontrolDetailController);

  /** @ngInject */
  function PmcontrolListController($uibModal,$log,$state,versionListService,toastr,moment,roleNav) {
    var vm = this;
    ...
    vm.open = function(version,module,name){
      ...
      var modalInstance  =  $uibModal.open({
        animation: true,
        templateUrl: 'app/pmcontrol/templates/reportText.html',
  ...
  /** @ngInject */
  function PmcontrolDetailController($uibModal,$log,$state,wecarListService,getVersionDetailService,toastr,moment,roleNav) {
    var vm = this;
    vm.maxSize = 5;
    vm.totalItems = 0;
    vm.currentPage = 1;
      ...
      if(e.target.checked){
        ...
        angular.forEach(vm.wecarList.data,function(value,index) {
          if(value.hasOwnProperty('destTasSysVer') && value.destTasSysVer === vm.versionDetail.version){
            $log.info('LOG[value.destTasSysVer];',value.destTasSysVer);
          }else{
        ...
        controller: function($scope,$uibModalInstance, items,$state,upgradeService){
          ...
          $scope.ok = function () {
            ...
            upgradeService.upgrade(items).then(function(data){
              $log.info('LOG[upgradeService.data]:',data);
              if(data.hasOwnProperty('err') && data.err === 0){
                var time = moment(new Date().getTime()).format("LTS").toString();
                toastr.success(data.msg+' time:'+time,"Hello "+roleNav.ename);
                $uibModalInstance.close();
