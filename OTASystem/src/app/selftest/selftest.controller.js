(function() {
  ...
  angular
    .module('otaRm')
    .controller('SelftestListController', SelftestListController)
    .controller('SelftestDetailController',SelftestDetailController);

  /** @ngInject */
  function SelftestListController($uibModal,$log,$state,versionListService,toastr,moment,roleNav) {
    var vm = this;
    ...
    vm.open = function(version,module,name){
      ...
      var modalInstance  =  $uibModal.open({
        animation: true,
        templateUrl: 'app/selftest/templates/reportText.html',
        controller: function($scope,$uibModalInstance, data,submitService){
          ...
          $scope.ok = function () {
            ...
            submitService.submit(data).then(function(data){
              $log.log("LOG[submitService]:",data);
  ...
  /** @ngInject */
  function SelftestDetailController($uibModal,$log,$state,wecarListService,getVersionDetailService,toastr,moment,roleNav) {
    var vm = this;
    vm.maxSize = 5;
    vm.totalItems = 0;
    vm.currentPage = 1;
    ...
    function activate(query){
      return getWecarList(query).then(function(){
        $log.log('LOG[vm.wecarList]:',vm.wecarIdList);
      ...
      if(e.target.checked){
        ...
        angular.forEach(vm.wecarList.data,function(value,index) {
          if(value.hasOwnProperty('destTasSysVer') && value.destTasSysVer === vm.versionDetail.version){
            $log.log('LOG[value.destTasSysVer]:',value.destTasSysVer);
          }else{
